...
#ifndef YSAsyncSocketLoggingEnabled
#define YSAsyncSocketLoggingEnabled 0
#endif

#if YSAsyncSocketLoggingEnabled

// Logging Enabled - See log level below

// Logging uses the CocoaLumberjack framework (which is also YS based).
...
#define LogAsync   YES
#define LogContext YSAsyncSocketLoggingContext
...
#ifndef YSAsyncSocketLogLevel
#define YSAsyncSocketLogLevel LOG_LEVEL_VERBOSE
#endif

// Log levels : off, error, warn, info, verbose
static const int logLevel = YSAsyncSocketLogLevel;
...
NSString *const YSAsyncSocketException = @"YSAsyncSocketException";
NSString *const YSAsyncSocketErrorDomain = @"YSAsyncSocketErrorDomain";

NSString *const YSAsyncSocketQueueName = @"YSAsyncSocket";
NSString *const YSAsyncSocketThreadName = @"YSAsyncSocket-CFStream";

NSString *const YSAsyncSocketManuallyEvaluateTrust = @"YSAsyncSocketManuallyEvaluateTrust";
#if TARGET_OS_IPHONE
NSString *const YSAsyncSocketUseCFStreamForTLS = @"YSAsyncSocketUseCFStreamForTLS";
#endif
NSString *const YSAsyncSocketSSLPeerID = @"YSAsyncSocketSSLPeerID";
NSString *const YSAsyncSocketSSLProtocolVersionMin = @"YSAsyncSocketSSLProtocolVersionMin";
NSString *const YSAsyncSocketSSLProtocolVersionMax = @"YSAsyncSocketSSLProtocolVersionMax";
NSString *const YSAsyncSocketSSLSessionOptionFalseStart = @"YSAsyncSocketSSLSessionOptionFalseStart";
NSString *const YSAsyncSocketSSLSessionOptionSendOneByteRecord = @"YSAsyncSocketSSLSessionOptionSendOneByteRecord";
NSString *const YSAsyncSocketSSLCipherSuites = @"YSAsyncSocketSSLCipherSuites";
#if !TARGET_OS_IPHONE
NSString *const YSAsyncSocketSSLDiffieHellmanParameters = @"YSAsyncSocketSSLDiffieHellmanParameters";
#endif

enum YSAsyncSocketFlags
{
...
@interface YSAsyncSocketPreBuffer : NSObject
{
	uint8_t *preBuffer;
...
@implementation YSAsyncSocketPreBuffer
...
/**
 * The YSAsyncReadPacket encompasses the instructions for any given read.
 * The content of a read packet allows the code to determine if we're:
 *  - reading to a certain length
 *  - reading to a certain separator
 *  - or simply reading the first chunk of available data
**/
@interface YSAsyncReadPacket : NSObject
{
  @public
...
- (NSUInteger)readLengthForTermWithPreBuffer:(YSAsyncSocketPreBuffer *)preBuffer found:(BOOL *)foundPtr;

- (NSInteger)searchForTermAfterPreBuffering:(ssize_t)numBytes;

@end

@implementation YSAsyncReadPacket
...
/**
 * The YSAsyncWritePacket encompasses the instructions for any given write.
**/
@interface YSAsyncWritePacket : NSObject
{
  @public
	NSData *buffer;
	NSUInteger bytesDone;
...
@implementation YSAsyncWritePacket
...
/**
 * The YSAsyncSpecialPacket encompasses special instructions for interruptions in the read/write queues.
 * This class my be altered to support more than just TLS in the future.
**/
@interface YSAsyncSpecialPacket : NSObject
{
  @public
...
@implementation YSAsyncSpecialPacket
...
@implementation YSAsyncSocket
{
	uint32_t flags;
	uint16_t config;
	
	__weak id<YSAsyncSocketDelegate> delegate;
	...
	YSAsyncReadPacket *currentRead;
	YSAsyncWritePacket *currentWrite;
	
	unsigned long socketFDBytesAvailable;
	
	YSAsyncSocketPreBuffer *preBuffer;
	...
	YSAsyncSocketPreBuffer *sslPreBuffer;
...
- (id)initWithDelegate:(id<YSAsyncSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	if((self = [super init]))
	{
		delegate = aDelegate;
		...
		{
			socketQueue = dispatch_queue_create([YSAsyncSocketQueueName UTF8String], NULL);
		}
		...
		preBuffer = [[YSAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<YSAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq error:(NSError**)error {
  return [self socketFromConnectedSocketFD:socketFD delegate:aDelegate delegateQueue:dq socketQueue:NULL error:error];
}

+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<YSAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq error:(NSError* __autoreleasing *)error
{
  __block BOOL errorOccured = NO;
  
  YSAsyncSocket *socket = [[[self class] alloc] initWithDelegate:aDelegate delegateQueue:dq socketQueue:sq];
  ...
  dispatch_sync(socket->socketQueue, ^{ @autoreleasepool {
    ...
    if (retVal)
    {
      NSString *errMsg = NSLocalizedStringWithDefaultValue(@"YSAsyncSocketOtherError",
                                                           @"YSAsyncSocket", [NSBundle mainBundle],
      ...
      errorOccured = YES;
      if (error)
        *error = [NSError errorWithDomain:YSAsyncSocketErrorDomain code:YSAsyncSocketOtherError userInfo:userInfo];
      return;
    }
...
- (void)getDelegate:(id<YSAsyncSocketDelegate> *)delegatePtr delegateQueue:(dispatch_queue_t *)delegateQueuePtr
{
	if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey))
	{
...
- (BOOL)doAccept:(int)parentSocketFD
{
	...
	if (delegateQueue)
	{
		...
		dispatch_async(delegateQueue, ^{ @autoreleasepool {
			...
			// Create YSAsyncSocket instance for accepted socket
			
			YSAsyncSocket *acceptedSocket = [[[self class] alloc] initWithDelegate:theDelegate
...
- (NSError *)badConfigError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:YSAsyncSocketErrorDomain code:YSAsyncSocketBadConfigError userInfo:userInfo];
}
...
- (NSError *)badParamError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:YSAsyncSocketErrorDomain code:YSAsyncSocketBadParamError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"YSAsyncSocketConnectTimeoutError",
	                                                     @"YSAsyncSocket", [NSBundle mainBundle],
	                                                     @"Attempt to connect to host timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:YSAsyncSocketErrorDomain code:YSAsyncSocketConnectTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"YSAsyncSocketReadMaxedOutError",
														 @"YSAsyncSocket", [NSBundle mainBundle],
														 @"Read operation reached set maximum length", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:YSAsyncSocketErrorDomain code:YSAsyncSocketReadMaxedOutError userInfo:info];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"YSAsyncSocketReadTimeoutError",
	                                                     @"YSAsyncSocket", [NSBundle mainBundle],
	                                                     @"Read operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:YSAsyncSocketErrorDomain code:YSAsyncSocketReadTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"YSAsyncSocketWriteTimeoutError",
	                                                     @"YSAsyncSocket", [NSBundle mainBundle],
	                                                     @"Write operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:YSAsyncSocketErrorDomain code:YSAsyncSocketWriteTimeoutError userInfo:userInfo];
}

- (NSError *)connectionClosedError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"YSAsyncSocketClosedError",
	                                                     @"YSAsyncSocket", [NSBundle mainBundle],
	                                                     @"Socket closed by remote peer", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:YSAsyncSocketErrorDomain code:YSAsyncSocketClosedError userInfo:userInfo];
}
...
- (NSError *)otherError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:YSAsyncSocketErrorDomain code:YSAsyncSocketOtherError userInfo:userInfo];
}
...
- (BOOL)usingCFStreamForTLS
{
	...
	{
		// The startTLS method was given the YSAsyncSocketUseCFStreamForTLS flag.
                        ...
                        tag:(long)tag
{
	...
	YSAsyncReadPacket *packet = [[YSAsyncReadPacket alloc] initWithData:buffer
	                                                          startOffset:offset
	                                                            maxLength:length
	                                                              timeout:timeout
	                                                           readLength:0
...
- (float)progressOfReadReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
        ...
        if (!self->currentRead || ![self->currentRead isKindOfClass:[YSAsyncReadPacket class]])
		{
...
- (void)maybeDequeueRead
{
	...
	if ((currentRead == nil) && (flags & kConnected))
	{
		if ([readQueue count] > 0)
		{
			...
			if ([currentRead isKindOfClass:[YSAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued YSAsyncSpecialPacket");
			...
			{
				LogVerbose(@"Dequeued YSAsyncReadPacket");
...
- (void)doReadData
{
	...
	if ([self usingCFStreamForTLS])
	{
		...
		// Requested CFStream, rather than SecureTransport, for TLS (via YSAsyncSocketUseCFStreamForTLS)
...
- (void)completeCurrentRead
{
	...
	{
		YSAsyncReadPacket *theRead = currentRead; // Ensure currentRead retained since result may not own buffer
...
- (void)doReadTimeout
{
	...
	{
		YSAsyncReadPacket *theRead = currentRead;
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	YSAsyncWritePacket *packet = [[YSAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (float)progressOfWriteReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
        ...
        if (!self->currentWrite || ![self->currentWrite isKindOfClass:[YSAsyncWritePacket class]])
		{
...
- (void)maybeDequeueWrite
{
	...
	if ((currentWrite == nil) && (flags & kConnected))
	{
		if ([writeQueue count] > 0)
		{
			...
			if ([currentWrite isKindOfClass:[YSAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued YSAsyncSpecialPacket");
			...
			{
				LogVerbose(@"Dequeued YSAsyncWritePacket");
...
- (void)doWriteData
{
	...
	// Note: This method is not called if currentWrite is a YSAsyncSpecialPacket (startTLS packet)
...
- (void)doWriteTimeout
{
	...
	{
		YSAsyncWritePacket *theWrite = currentWrite;
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
	...
	YSAsyncSpecialPacket *packet = [[YSAsyncSpecialPacket alloc] initWithTLSSettings:tlsSettings];
...
- (void)maybeStartTLS
{
	...
	if ((flags & kStartingReadTLS) && (flags & kStartingWriteTLS))
	{
		...
		#if TARGET_OS_IPHONE
		{
			YSAsyncSpecialPacket *tlsPacket = (YSAsyncSpecialPacket *)currentRead;
            NSDictionary *tlsSettings = @{};
            if (tlsPacket) {
                tlsSettings = tlsPacket->tlsSettings;
            }
			NSNumber *value = [tlsSettings objectForKey:YSAsyncSocketUseCFStreamForTLS];
...
{
	YSAsyncSocket *asyncSocket = (__bridge YSAsyncSocket *)connection;
...
- (void)ssl_startTLS
{
	...
	BOOL shouldManuallyEvaluateTrust = [[tlsSettings objectForKey:YSAsyncSocketManuallyEvaluateTrust] boolValue];
	...
	//  3. YSAsyncSocketSSLPeerID
	//  4. YSAsyncSocketSSLProtocolVersionMin
	//  5. YSAsyncSocketSSLProtocolVersionMax
	//  6. YSAsyncSocketSSLSessionOptionFalseStart
	//  7. YSAsyncSocketSSLSessionOptionSendOneByteRecord
	//  8. YSAsyncSocketSSLCipherSuites
	//  9. YSAsyncSocketSSLDiffieHellmanParameters (Mac)
	//
	...
	value = [tlsSettings objectForKey:YSAsyncSocketSSLPeerID];
	...
	{
		NSAssert(NO, @"Invalid value for YSAsyncSocketSSLPeerID. Value must be of type NSData."
		             @" (You can convert strings to data using a method like"
		             @" [string dataUsingEncoding:NSUTF8StringEncoding])");
		
		[self closeWithError:[self otherError:@"Invalid value for YSAsyncSocketSSLPeerID."]];
		return;
	}
	
	// 4. YSAsyncSocketSSLProtocolVersionMin
	
	value = [tlsSettings objectForKey:YSAsyncSocketSSLProtocolVersionMin];
	...
	{
		NSAssert(NO, @"Invalid value for YSAsyncSocketSSLProtocolVersionMin. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for YSAsyncSocketSSLProtocolVersionMin."]];
		return;
	}
	
	// 5. YSAsyncSocketSSLProtocolVersionMax
	
	value = [tlsSettings objectForKey:YSAsyncSocketSSLProtocolVersionMax];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for YSAsyncSocketSSLProtocolVersionMax. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for YSAsyncSocketSSLProtocolVersionMax."]];
		return;
	}
	
	// 6. YSAsyncSocketSSLSessionOptionFalseStart
	
	value = [tlsSettings objectForKey:YSAsyncSocketSSLSessionOptionFalseStart];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for YSAsyncSocketSSLSessionOptionFalseStart. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for YSAsyncSocketSSLSessionOptionFalseStart."]];
		return;
	}
	
	// 7. YSAsyncSocketSSLSessionOptionSendOneByteRecord
	
	value = [tlsSettings objectForKey:YSAsyncSocketSSLSessionOptionSendOneByteRecord];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for YSAsyncSocketSSLSessionOptionSendOneByteRecord."
		             @" Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for YSAsyncSocketSSLSessionOptionSendOneByteRecord."]];
		return;
	}
	
	// 8. YSAsyncSocketSSLCipherSuites
	
	value = [tlsSettings objectForKey:YSAsyncSocketSSLCipherSuites];
	if ([value isKindOfClass:[NSArray class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for YSAsyncSocketSSLCipherSuites. Value must be of type NSArray.");
		
		[self closeWithError:[self otherError:@"Invalid value for YSAsyncSocketSSLCipherSuites."]];
		return;
	}
	
	// 9. YSAsyncSocketSSLDiffieHellmanParameters
	
	#if !TARGET_OS_IPHONE
	value = [tlsSettings objectForKey:YSAsyncSocketSSLDiffieHellmanParameters];
	if ([value isKindOfClass:[NSData class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for YSAsyncSocketSSLDiffieHellmanParameters. Value must be of type NSData.");
		
		[self closeWithError:[self otherError:@"Invalid value for YSAsyncSocketSSLDiffieHellmanParameters."]];
		return;
	}
	#endif
	...
	if (value)
	{
		NSAssert(NO, @"Security option unavailable - kCFStreamSSLLevel"
		             @" - You must use YSAsyncSocketSSLProtocolVersionMin & YSAsyncSocketSSLProtocolVersionMax");
	...
	sslPreBuffer = [[YSAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
- (void)ssl_continueSSLHandshake
{
	...
	else if (status == errSSLPeerAuthCompleted)
	{
		...
		else
		{
			...
			NSString *msg = @"YSAsyncSocketManuallyEvaluateTrust specified in tlsSettings,"
...
- (void)cf_startTLS
{
	...
	NSAssert([currentRead isKindOfClass:[YSAsyncSpecialPacket class]], @"Invalid read packet for startTLS");
	NSAssert([currentWrite isKindOfClass:[YSAsyncSpecialPacket class]], @"Invalid write packet for startTLS");
...
+ (void)startCFStreamThreadIfNeeded
{
	...
	dispatch_once(&predicate, ^{
		...
		cfstreamThreadSetupQueue = dispatch_queue_create("YSAsyncSocket-CFStreamThreadSetup", DISPATCH_QUEUE_SERIAL);
	});
...
{
	[[NSThread currentThread] setName:YSAsyncSocketThreadName];
...
+ (void)scheduleCFStreams:(YSAsyncSocket *)asyncSocket
{
	LogTrace();
...
+ (void)unscheduleCFStreams:(YSAsyncSocket *)asyncSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == cfstreamThread, @"Invoked on wrong thread");
...
{
	YSAsyncSocket *asyncSocket = (__bridge YSAsyncSocket *)pInfo;
