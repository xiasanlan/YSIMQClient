...
@class YSAsyncReadPacket;
@class YSAsyncWritePacket;
@class YSAsyncSocketPreBuffer;
@protocol YSAsyncSocketDelegate;

NS_ASSUME_NONNULL_BEGIN

extern NSString *const YSAsyncSocketException;
extern NSString *const YSAsyncSocketErrorDomain;

extern NSString *const YSAsyncSocketQueueName;
extern NSString *const YSAsyncSocketThreadName;

extern NSString *const YSAsyncSocketManuallyEvaluateTrust;
#if TARGET_OS_IPHONE
extern NSString *const YSAsyncSocketUseCFStreamForTLS;
#endif
#define YSAsyncSocketSSLPeerName     (NSString *)kCFStreamSSLPeerName
#define YSAsyncSocketSSLCertificates (NSString *)kCFStreamSSLCertificates
#define YSAsyncSocketSSLIsServer     (NSString *)kCFStreamSSLIsServer
extern NSString *const YSAsyncSocketSSLPeerID;
extern NSString *const YSAsyncSocketSSLProtocolVersionMin;
extern NSString *const YSAsyncSocketSSLProtocolVersionMax;
extern NSString *const YSAsyncSocketSSLSessionOptionFalseStart;
extern NSString *const YSAsyncSocketSSLSessionOptionSendOneByteRecord;
extern NSString *const YSAsyncSocketSSLCipherSuites;
#if !TARGET_OS_IPHONE
extern NSString *const YSAsyncSocketSSLDiffieHellmanParameters;
#endif

#define YSAsyncSocketLoggingContext 65535


typedef NS_ENUM(NSInteger, YSAsyncSocketError) {
	YSAsyncSocketNoError = 0,           // Never used
	YSAsyncSocketBadConfigError,        // Invalid configuration
	YSAsyncSocketBadParamError,         // Invalid parameter was passed
	YSAsyncSocketConnectTimeoutError,   // A connect operation timed out
	YSAsyncSocketReadTimeoutError,      // A read operation timed out
	YSAsyncSocketWriteTimeoutError,     // A write operation timed out
	YSAsyncSocketReadMaxedOutError,     // Reached set maxLength without completing
	YSAsyncSocketClosedError,           // The remote peer closed the connection
	YSAsyncSocketOtherError,            // Description provided in userInfo
};
...
@interface YSAsyncSocket : NSObject

/**
 * YSAsyncSocket uses the standard delegate paradigm,
 ...
 * If you pass NULL, YSAsyncSocket will automatically create it's own socket queue.
...
- (instancetype)initWithDelegate:(nullable id<YSAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq;
- (instancetype)initWithDelegate:(nullable id<YSAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq;

/**
 * Create YSAsyncSocket from already connect BSD socket file descriptor
**/
...
+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<YSAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq error:(NSError **)error;

#pragma mark Configuration

@property (atomic, weak, readwrite, nullable) id<YSAsyncSocketDelegate> delegate;
...
- (void)getDelegate:(id<YSAsyncSocketDelegate> __nullable * __nullable)delegatePtr delegateQueue:(dispatch_queue_t __nullable * __nullable)delegateQueuePtr;
- (void)setDelegate:(nullable id<YSAsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;
...
**/
- (void)synchronouslySetDelegate:(nullable id<YSAsyncSocketDelegate>)delegate;
- (void)synchronouslySetDelegateQueue:(nullable dispatch_queue_t)delegateQueue;
- (void)synchronouslySetDelegate:(nullable id<YSAsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;

/**
 * By default, both IPv4 and IPv6 are enabled.
 * 
 * For accepting incoming connections, this means YSAsyncSocket automatically supports both protocols,
 * and can simulataneously accept incoming connections on either protocol.
 * 
 * For outgoing connections, this means YSAsyncSocket can connect to remote hosts running either protocol.
 * If a DNS lookup returns only IPv4 results, YSAsyncSocket will automatically use IPv4.
 * If a DNS lookup returns only IPv6 results, YSAsyncSocket will automatically use IPv6.
 ...
 * When a connection is accepted, a new instance of YSAsyncSocket will be spawned to handle it,
 ...
 * 
 * Please note the recommended way of releasing a YSAsyncSocket instance (e.g. in a dealloc method)
 ...
 * it is treated similarly to a timeout - the socket is closed with a YSAsyncSocketReadMaxedOutError.
 ...
 * This is due to the fact that YSAsyncSocket does NOT copy the data. It simply retains it.
 ...
 * 
 * - YSAsyncSocketManuallyEvaluateTrust
 ...
 *     So YSAsyncSocket will invoke the delegate method socket:shouldTrustPeer: passing the SecTrustRef.
 *
 ...
 *
 * - YSAsyncSocketUseCFStreamForTLS (iOS only)
 *     The value must be of type NSNumber, encapsulating a BOOL value.
 *     By default YSAsyncSocket will use the SecureTransport layer to perform encryption.
 ...
 *     
 *     However, if you absolutely must, you can instruct YSAsyncSocket to use the old-fashioned encryption
 *     technique by going through the CFStream instead. So instead of using SecureTransport, YSAsyncSocket
 ...
 *     
 *     Thus all the other keys in the given dictionary will be ignored by YSAsyncSocket,
 ...
 *
 * - YSAsyncSocketSSLProtocolVersionMin
 * - YSAsyncSocketSSLProtocolVersionMax
 ...
 *
 * - YSAsyncSocketSSLDiffieHellmanParameters (Mac OS X only)
 *     The value must be of type NSData.
 ...
 *     You MUST use manual trust evaluation instead (see YSAsyncSocketManuallyEvaluateTrust).
 ...
 *     You MUST use YSAsyncSocketSSLProtocolVersionMin & YSAsyncSocketSSLProtocolVersionMin instead.
...
/**
 * YSAsyncSocket maintains thread safety by using an internal serial dispatch_queue.
 ...
 * This is because the YS API offers no mechanism to discover a queue's targetQueue.
 ...
 * - (dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(YSAsyncSocket *)sock
 * {
 ...
 *     return socketQueue;
 * }
 * - (void)socket:(YSAsyncSocket *)sock didAcceptNewSocket:(YSAsyncSocket *)newSocket
 * {
 ...
 *   The faster and more powerful YS API isn't notified properly in this case.
 * 
...
@protocol YSAsyncSocketDelegate <NSObject>
@optional
...
**/
- (nullable dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(YSAsyncSocket *)sock;
...
**/
- (void)socket:(YSAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
 ...
 * It may be used to for things such as updating progress bars.
**/
- (void)socket:(YSAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)socket:(YSAsyncSocket *)sock shouldTimeoutReadWithTag:(long)tag
 ...
 * 
 * Note: If the YSAsyncSocket instance is deallocated while it is still connected,
 ...
 * - YSAsyncSocketManuallyEvaluateTrust == YES
 *
