...
extern NSString *const YSAsyncUdpSocketException;
extern NSString *const YSAsyncUdpSocketErrorDomain;

extern NSString *const YSAsyncUdpSocketQueueName;
extern NSString *const YSAsyncUdpSocketThreadName;

typedef NS_ENUM(NSInteger, YSAsyncUdpSocketError) {
	YSAsyncUdpSocketNoError = 0,          // Never used
	YSAsyncUdpSocketBadConfigError,       // Invalid configuration
	YSAsyncUdpSocketBadParamError,        // Invalid parameter was passed
	YSAsyncUdpSocketSendTimeoutError,     // A send operation timed out
	YSAsyncUdpSocketClosedError,          // The socket was closed
	YSAsyncUdpSocketOtherError,           // Description provided in userInfo
};
...
@class YSAsyncUdpSocket;

@protocol YSAsyncUdpSocketDelegate <NSObject>
@optional
...
**/
- (void)udpSocket:(YSAsyncUdpSocket *)sock didConnectToAddress:(NSData *)address;
...
**/
- (void)udpSocket:(YSAsyncUdpSocket *)sock didNotConnect:(NSError * _Nullable)error;

/**
 * Called when the datagram with the given tag has been sent.
**/
- (void)udpSocket:(YSAsyncUdpSocket *)sock didSendDataWithTag:(long)tag;

/**
 * Called if an error occurs while trying to send a datagram.
 * This could be due to a timeout, or something more serious such as the data being too large to fit in a sigle packet.
**/
- (void)udpSocket:(YSAsyncUdpSocket *)sock didNotSendDataWithTag:(long)tag dueToError:(NSError * _Nullable)error;

/**
 * Called when the socket has received the requested datagram.
**/
- (void)udpSocket:(YSAsyncUdpSocket *)sock didReceiveData:(NSData *)data
...
**/
- (void)udpSocketDidClose:(YSAsyncUdpSocket *)sock withError:(NSError  * _Nullable)error;
 ...
 * Example:
 * 
 * YSAsyncUdpSocketReceiveFilterBlock filter = ^BOOL (NSData *data, NSData *address, id *context) {
 * 
 ...
 * 
**/
typedef BOOL (^YSAsyncUdpSocketReceiveFilterBlock)(NSData *data, NSData *address, id __nullable * __nonnull context);
 ...
 *
**/
typedef BOOL (^YSAsyncUdpSocketSendFilterBlock)(NSData *data, NSData *address, long tag);


@interface YSAsyncUdpSocket : NSObject

/**
 * YSAsyncUdpSocket uses the standard delegate paradigm,
 ...
 * If you pass NULL, YSAsyncSocket will automatically create its own socket queue.
...
- (instancetype)initWithDelegate:(nullable id <YSAsyncUdpSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq;
- (instancetype)initWithDelegate:(nullable id <YSAsyncUdpSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq;

#pragma mark Configuration

- (nullable id <YSAsyncUdpSocketDelegate>)delegate;
- (void)setDelegate:(nullable id <YSAsyncUdpSocketDelegate>)delegate;
- (void)synchronouslySetDelegate:(nullable id <YSAsyncUdpSocketDelegate>)delegate;
...
- (void)getDelegate:(id <YSAsyncUdpSocketDelegate> __nullable * __nullable)delegatePtr delegateQueue:(dispatch_queue_t __nullable * __nullable)delegateQueuePtr;
- (void)setDelegate:(nullable id <YSAsyncUdpSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;
- (void)synchronouslySetDelegate:(nullable id <YSAsyncUdpSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;

/**
 * By default, both IPv4 and IPv6 are enabled.
 * 
 * This means YSAsyncUdpSocket automatically supports both protocols,
 * and can send to IPv4 or IPv6 addresses,
 * as well as receive over IPv4 and IPv6.
 * 
 * For operations that require DNS resolution, YSAsyncUdpSocket supports both IPv4 and IPv6.
 * If a DNS lookup returns only IPv4 results, YSAsyncUdpSocket will automatically use IPv4.
 * If a DNS lookup returns only IPv6 results, YSAsyncUdpSocket will automatically use IPv6.
 ...
 * 
 * Since the OS/YS notifies us of the size of each received UDP packet,
 ...
 * This is due to the fact that YSAsyncUdpSocket does NOT copy the data.
 ...
 *    or the system tools to handle this aren't available (e.g. on a mobile device).
 * 
 * For more information about YSAsyncUdpSocketSendFilterBlock, see the documentation for its typedef.
...
**/
- (void)setSendFilter:(nullable YSAsyncUdpSocketSendFilterBlock)filterBlock withQueue:(nullable dispatch_queue_t)filterQueue;
...
**/
- (void)setSendFilter:(nullable YSAsyncUdpSocketSendFilterBlock)filterBlock
 ...
 * 
 * Important Note:
 * YSAsyncUdpSocket may be running in parallel with your code.
 * That is, your delegate is likely running on a separate thread/dispatch_queue.
 * When you invoke this method, YSAsyncUdpSocket may have already dispatched delegate methods to be invoked.
 ...
 * [udpSocket setReceiveFilter:filter withQueue:myParsingQueue];
 * 
 * For more information about YSAsyncUdpSocketReceiveFilterBlock, see the documentation for its typedef.
 * To remove a previously set filter, invoke this method and pass a nil filterBlock and NULL filterQueue.
 * 
 * Note: This method invokes setReceiveFilter:withQueue:isAsynchronous: (documented below),
 *       passing YES for the isAsynchronous parameter.
**/
- (void)setReceiveFilter:(nullable YSAsyncUdpSocketReceiveFilterBlock)filterBlock withQueue:(nullable dispatch_queue_t)filterQueue;
 ...
 * For example, you can't query properties on the socket.
**/
- (void)setReceiveFilter:(nullable YSAsyncUdpSocketReceiveFilterBlock)filterBlock
               withQueue:(nullable dispatch_queue_t)filterQueue
          isAsynchronous:(BOOL)isAsynchronous;
 ...
 * 
 * The YSAsyncUdpSocket instance may optionally be used again.
 ...
 * 
 * Generally YSAsyncUdpSocket doesn't use CFStream. (It uses the faster YS API's.)
