...
@class YSAsyncUdpSendPacket;

NSString *const YSAsyncUdpSocketException = @"YSAsyncUdpSocketException";
NSString *const YSAsyncUdpSocketErrorDomain = @"YSAsyncUdpSocketErrorDomain";

NSString *const YSAsyncUdpSocketQueueName = @"YSAsyncUdpSocket";
NSString *const YSAsyncUdpSocketThreadName = @"YSAsyncUdpSocket-CFStream";

enum YSAsyncUdpSocketFlags
{
...
@interface YSAsyncUdpSocket ()
{
	...
	YSAsyncUdpSocketReceiveFilterBlock receiveFilterBlock;
	dispatch_queue_t receiveFilterQueue;
	BOOL receiveFilterAsync;
	
	YSAsyncUdpSocketSendFilterBlock sendFilterBlock;
	...
	YSAsyncUdpSendPacket *currentSend;
...
/**
 * The YSAsyncUdpSendPacket encompasses the instructions for a single send/write.
**/
@interface YSAsyncUdpSendPacket : NSObject {
@public
	NSData *buffer;
...
@implementation YSAsyncUdpSendPacket
...
@interface YSAsyncUdpSpecialPacket : NSObject {
@public
//	uint8_t type;
...
@implementation YSAsyncUdpSpecialPacket
...
@implementation YSAsyncUdpSocket
...
- (id)initWithDelegate:(id <YSAsyncUdpSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq
{
	LogTrace();
...
- (id)initWithDelegate:(id <YSAsyncUdpSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	LogTrace();
	...
	if ((self = [super init]))
	{
		...
		{
			socketQueue = dispatch_queue_create([YSAsyncUdpSocketQueueName UTF8String], NULL);
		}
...
- (void)setDelegate:(id <YSAsyncUdpSocketDelegate>)newDelegate synchronously:(BOOL)synchronously
{
...
- (void)setDelegate:(id <YSAsyncUdpSocketDelegate>)newDelegate
{
	[self setDelegate:newDelegate synchronously:NO];
}

- (void)synchronouslySetDelegate:(id <YSAsyncUdpSocketDelegate>)newDelegate
{
...
- (void)getDelegate:(id <YSAsyncUdpSocketDelegate> *)delegatePtr delegateQueue:(dispatch_queue_t *)delegateQueuePtr
{
	if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey))
	{
...
- (void)setDelegate:(id <YSAsyncUdpSocketDelegate>)newDelegate delegateQueue:(dispatch_queue_t)newDelegateQueue synchronously:(BOOL)synchronously
{
	dispatch_block_t block = ^{
...
- (void)setDelegate:(id <YSAsyncUdpSocketDelegate>)newDelegate delegateQueue:(dispatch_queue_t)newDelegateQueue
{
	[self setDelegate:newDelegate delegateQueue:newDelegateQueue synchronously:NO];
}

- (void)synchronouslySetDelegate:(id <YSAsyncUdpSocketDelegate>)newDelegate delegateQueue:(dispatch_queue_t)newDelegateQueue
{
...
- (NSError *)badConfigError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:YSAsyncUdpSocketErrorDomain
	                           code:YSAsyncUdpSocketBadConfigError
	                       userInfo:userInfo];
}
...
- (NSError *)badParamError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:YSAsyncUdpSocketErrorDomain
	                           code:YSAsyncUdpSocketBadParamError
	                       userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"YSAsyncUdpSocketSendTimeoutError",
	                                                     @"YSAsyncUdpSocket", [NSBundle mainBundle],
	                                                     @"Send operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:YSAsyncUdpSocketErrorDomain
	                           code:YSAsyncUdpSocketSendTimeoutError
	                       userInfo:userInfo];
}

- (NSError *)socketClosedError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"YSAsyncUdpSocketClosedError",
	                                                     @"YSAsyncUdpSocket", [NSBundle mainBundle],
	                                                     @"Socket closed", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:YSAsyncUdpSocketErrorDomain code:YSAsyncUdpSocketClosedError userInfo:userInfo];
}
...
- (NSError *)otherError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:YSAsyncUdpSocketErrorDomain
	                           code:YSAsyncUdpSocketOtherError
	                       userInfo:userInfo];
}
...
- (BOOL)createSocket4:(BOOL)useIPv4 socket6:(BOOL)useIPv6 error:(NSError * __autoreleasing *)errPtr
{
	...
	int(^createSocket)(int) = ^int (int domain) {
         ...
         *
         * This is the reason of #222(YS does not necessarily return the size of an entire UDP packet) and
         *  #535(YSAsyncUDPSocket can not send data when data is greater than 9K)
         *
         *
...
- (BOOL)connectToHost:(NSString *)host onPort:(uint16_t)port error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		YSAsyncUdpSpecialPacket *packet = [[YSAsyncUdpSpecialPacket alloc] init];
...
- (void)maybeConnect
{
	...
	BOOL sendQueueReady = [currentSend isKindOfClass:[YSAsyncUdpSpecialPacket class]];
	
	if (sendQueueReady)
	{
		YSAsyncUdpSpecialPacket *connectPacket = (YSAsyncUdpSpecialPacket *)currentSend;
...
- (void)sendData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	YSAsyncUdpSendPacket *packet = [[YSAsyncUdpSendPacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (void)sendData:(NSData *)data toAddress:(NSData *)remoteAddr withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	YSAsyncUdpSendPacket *packet = [[YSAsyncUdpSendPacket alloc] initWithData:data timeout:timeout tag:tag];
	packet->addressFamily = [YSAsyncUdpSocket familyFromAddress:remoteAddr];
...
- (void)setSendFilter:(YSAsyncUdpSocketSendFilterBlock)filterBlock withQueue:(dispatch_queue_t)filterQueue
{
	[self setSendFilter:filterBlock withQueue:filterQueue isAsynchronous:YES];
}

- (void)setSendFilter:(YSAsyncUdpSocketSendFilterBlock)filterBlock
            withQueue:(dispatch_queue_t)filterQueue
       isAsynchronous:(BOOL)isAsynchronous
{
	YSAsyncUdpSocketSendFilterBlock newFilterBlock = NULL;
...
- (void)maybeDequeueSend
{
	...
	if (currentSend == nil)
	{
		...
		while ([sendQueue count] > 0)
		{
			...
			if ([currentSend isKindOfClass:[YSAsyncUdpSpecialPacket class]])
			{
				[self maybeConnect];
...
- (void)doPreSend
{
	...
	if (sendFilterBlock && sendFilterQueue)
	{
		...
		if (sendFilterAsync)
		{
			...
			YSAsyncUdpSendPacket *sendPacket = currentSend;
...
- (void)setReceiveFilter:(YSAsyncUdpSocketReceiveFilterBlock)filterBlock withQueue:(dispatch_queue_t)filterQueue
{
	[self setReceiveFilter:filterBlock withQueue:filterQueue isAsynchronous:YES];
}

- (void)setReceiveFilter:(YSAsyncUdpSocketReceiveFilterBlock)filterBlock
               withQueue:(dispatch_queue_t)filterQueue
          isAsynchronous:(BOOL)isAsynchronous
{
	YSAsyncUdpSocketReceiveFilterBlock newFilterBlock = NULL;
	dispatch_queue_t newFilterQueue = NULL;
...
- (void)doReceive
{
	...
	if (doReceive4)
	{
		...
		// #222: YS does not necessarily return the size of an entire UDP packet 
...
+ (void)listenerThread
{
	@autoreleasepool {
		...
		[[NSThread currentThread] setName:YSAsyncUdpSocketThreadName];
...
+ (void)addStreamListener:(YSAsyncUdpSocket *)asyncUdpSocket
{
	LogTrace();
...
+ (void)removeStreamListener:(YSAsyncUdpSocket *)asyncUdpSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == listenerThread, @"Invoked on wrong thread");
...
{
	@autoreleasepool {
		YSAsyncUdpSocket *asyncUdpSocket = (__bridge YSAsyncUdpSocket *)pInfo;
